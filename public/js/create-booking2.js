...
$('.addItemToList').click(function() {
	...
	var itemRateAddHour = $(this).closest('tr').find('.itemRateAddHour').val();
	var itemRateAddHourNumbered = Number(itemRateAddHour);
	var itemRateAddHourSimplified = itemRateAddHour.replace(/\.00$/,'');
	
	var additionalDayTotal = itemRateAddHourNumbered * amountOfHoursMinusOne;
	var itemTotal = (additionalDayTotal + itemRateFirstHourNumbered) * itemQuantity;
	...
	if ( itemQuantity == 0 ) {
		...
		$('.itemsList').append(
		'<li class="inputedItem create-booking__cart__item">'
			+ '<span class="create-booking__cart__item__name">' + itemTitle + '</span>'
			+ '<span class="create-booking__cart__item__rate">' + '$' + itemRateFirstHourSimplified + '/' + itemRateAddHourSimplified + '</span>'
			+ '<span class="create-booking__cart__item__quantity">' + itemQuantity + '</span>'
			+ '<span class="create-booking__cart__item__total">' + '$' + itemTotalDecimal + '</span>'
			+ '<span class="create-booking__cart__item__remove">' + '<i class="fas fa-times create-booking__cart__item__remove__x"></i>' + '</span>'
		+ '</li>' );
		...
		itemArray = [itemTitle, itemQuantity, amountOfHours, itemRateFirstHour, itemRateAddHour, itemTotal];
		...
		$('.create-booking__cart__total__number').text(itemsSum);
...
$(document).on('click', '.create-booking__cart__item__remove__x', function() {
	
	// Get the name of the item to cancel
	var nameOfItemToCancel = $(this).closest('.create-booking__cart__item').find('.create-booking__cart__item__name').text();
	
	// Get the price total of the item to cancel and turn it into a number
	var totalOfItemToCancel = $(this).closest('.create-booking__cart__item').find('.create-booking__cart__item__total').text();
	...
	$(this).closest('.create-booking__cart__item').remove();
...
$('.createNewBookingSubmit').click(function() {
	...
	if (itemsArray.length == 0) {
		...
		document.getElementById("submit-booking").submit();
	}
});
...
function myFunction() {
  ...
  for (i = 0; i < tr.length; i++) {
	...
	var allPossibleItems = document.querySelectorAll('.create-booking__add-items__item-row');
